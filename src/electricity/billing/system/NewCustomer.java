    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == next) {
            ...
            try {
                ...
                c.s.executeUpdate(query1); // Insert into customer table
                c.s.executeUpdate(query2); // Insert into login table

                JOptionPane.showMessageDialog(null, "Customer Details Added Successfully");
                setVisible(false); // Close current window

                // Open MeterInfo window
                new MeterInfo(meter);
            ...
            setVisible(false); // Close if Cancel is clicked
        }
    }

    public static void main(String[] args) {
        new NewCustomer(); // Launch the form
    }
}
